...
public class RobotAlignToSpeaker : MonoBehaviour, IResettable
{
    ...
    [SerializeField] private float ampAngle;
    [SerializeField] private bool robotHasUniquePassAngle;
    [SerializeField] private bool dontAllowShootingWhenStowed = true;
    [SerializeField] private bool robotStowsToIntake;
    [SerializeField] private bool is1678;
    [SerializeField] private bool is1778;
    [SerializeField] private GameObject ampBar;
    [SerializeField] private float ampBarRotation;
    
    private Quaternion _ampBarStartingRot;

    public RobotNoteManager ringCollisions;

    [SerializeField] private float passingAngle;

    public Vector3 shooterRotationSetpoint;
    ...
    private void Start()
    {
        ...
        target = alliance == Alliance.Blue ? GameObject.Find("BlueAutoAlignTarget").transform : GameObject.Find("RedAutoAlignTarget").transform;

        if (robot == RobotSettings.ChillOut)
        {
            _ampBarStartingRot = ampBar.gameObject.transform.localRotation;
        }
    }
            ...
            if (!isTrapping) 
            {
                ...
                if (!isAmping && !isPassing)
                {
                    ...
                    else if (amp && ringCollisions.hasRingInRobot && !isAmping && !ringCollisions.isShooting && robotThatRotatesUpForAmping)
                    {
                        ...
                        {
                            StartCoroutine(AmplifyRotation1778());
                        }
                    }
                    ...
                    if (drive.isIntaking && robotStowsToIntake)
                    {
                        StowShooter();
                    } else if (robotStowsToIntake)
                    {
                        StartCoroutine(UnstowShooter());
                    }
                }
    ...
    private IEnumerator AmplifyRotation1778()
    {
        isAmping = true;

        Quaternion startRotation = shooterPivot.transform.localRotation;
        Quaternion ampBarStartRot = ampBar.gameObject.transform.localRotation;

        Quaternion targetRotation = Quaternion.Euler(ampAngle, 0f, 0f);
        Quaternion ampBarTargetRot = Quaternion.Euler(ampBarRotation, 0f, 0f);
        ...
        {
            shooterPivot.transform.localRotation = Quaternion.Slerp(startRotation, targetRotation, elapsedTime / duration);
            ampBar.transform.localRotation = Quaternion.Slerp(ampBarStartRot, ampBarTargetRot, elapsedTime / duration);
            elapsedTime += Time.deltaTime;
            yield return null;
        }

        shooterPivot.transform.localRotation = targetRotation;
        ampBar.transform.localRotation = ampBarTargetRot;
        ...
        {
            shooterPivot.transform.localRotation = Quaternion.Slerp(targetRotation, startRotation, elapsedTime / duration);
            ampBar.transform.localRotation = Quaternion.Slerp(ampBarTargetRot, ampBarStartRot, elapsedTime / duration);
            elapsedTime += Time.deltaTime;
            yield return null;
        }

        shooterPivot.transform.localRotation = startRotation;
        ampBar.transform.localRotation = ampBarStartRot;
    ...
    private void RotateShooter()
    {
        ...
        if (shooterPivot.transform.localEulerAngles.x >= hardMinAngle && shooterPivot.transform.localEulerAngles.x <= hardMaxAngle) 
        {
            ...
            // {
                ...
                shooterRotationSetpoint = shooterPivot.transform.localRotation.eulerAngles;
            // }
        }
        else
        {
